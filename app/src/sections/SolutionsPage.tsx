...
const solutions = [
  {
    ...
    fullDescription: '基于 UniAuth 统一身份与访问管理（IAM）平台，为企业提供面向远程终端、本地终端及应用系统的统一认证与准入能力。方案支持多种身份认证方式与标准协议，可作为终端准入认证、应用系统认证及统一身份入口，实现对用户、设备与应用访问行为的集中管理与安全控制。',
    features: ['远程终端', '本地终端', '应用系统', '终端准入', '应用认证'],
    capabilities: [
      {
        icon: Database,
        title: '多种 IdP 支持',
        items: ['AD, LDAP', 'SAML: Azure AD, Google Workspace', 'OAuth 2.0/OIDC: 飞书、钉钉、企微'],
      },
      {
        icon: Key,
        title: '认证安全措施',
        items: ['密码强度/周期策略', '多因素认证(MFA)', '通行密钥(Passkey)'],
      },
      {
        icon: Fingerprint,
        title: '多端身份互通',
        items: ['客户端到设备', '设备到设备', '应用单点登录'],
      },
    ],
  },
  {
    ...
    fullDescription: 'UniSASE 面向企业多分支、多云与全球化业务场景，提供基于 UniSASE 平台的统一接入与智能组网能力，替代传统专线与复杂网络架构。方案支持总部、分支、数据中心、云资源及远程用户的统一接入，通过就近接入、智能选路与链路优化，保障关键业务访问的稳定性与一致体验。',
    features: ['全球骨干网', '智能选路', '隐藏暴露面', '广域网组网', 'Wi-Fi 接入'],
    capabilities: [
      {
        icon: Globe,
        title: '质量保证',
        items: ['全球骨干网及 PoP 点', '基于应用的智能选路', 'SLA 自动切换'],
      },
      {
        icon: Shield,
        title: '减少暴露面',
              ...
              {currentSolution.capabilities.map((cap, index) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h3 className="text-lg font-bold text-brand-dark">{cap.title}</h3>
                  </div>
