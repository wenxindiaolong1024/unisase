...
vi.mock('lucide-react', () => ({
  ...
  Zap: ({ className }: { className?: string }) => <svg data-testid="zap" className={className} />,
...
describe('Features Component', () => {
  it('should render section header', () => {
    ...
    expect(screen.getByText('UniSASE 主要功能')).toBeInTheDocument();
  });

  it('should render main heading', () => {
    render(<Features />);

    expect(screen.getByText('全栈网络与安全能力')).toBeInTheDocument();
  });

  it('should render description', () => {
    render(<Features />);

    expect(screen.getByText(/从身份认证到策略控制，从安全防护到日志分析/)).toBeInTheDocument();
  });
  ...
  it('should render all sub features', () => {
    ...
    expect(screen.getByText('应用识别')).toBeInTheDocument();
  ...
  it('should render CTA button', () => {
    ...
    const ctaButton = screen.getByRole('button', { name: /了解更多功能详情/ });
    expect(ctaButton).toBeInTheDocument();
  });

  it('should render Zap icon in CTA button', () => {
    render(<Features />);

    const zapIcon = screen.getByTestId('zap');
  ...
  it('should render main features in grid layout', () => {
    ...
    const grid = container.querySelector('.lg\\:cols-3');
  ...
  it('should render sub features in correct order', () => {
    ...
    const subFeatures = screen.getAllByText(/SD-WAN|零信任|全球接入|威胁情报|应用识别|防火墙/);
