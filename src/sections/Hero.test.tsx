...
describe('Hero Component', () => {
  ...
  it('should render stats correctly', () => {
    ...
    expect(screen.getByText('服务可用性')).toBeInTheDocument();
  });

  it('should render CTA button when showCTA is true', () => {
    render(<Hero showCTA={true} />);

    const ctaButton = screen.getByRole('button', { name: /申请试用/ });
  ...
  it('should not render CTA button when showCTA is false', () => {
    ...
    const ctaButton = screen.queryByRole('button', { name: /申请试用/ });
  ...
  it('should call window.location.href on CTA button click', () => {
    ...
    expect(window.location.href).toBe('mailto:business@unisase.cn');
  });

  it('should render shield icon', () => {
    render(<Hero />);

    const shieldIcon = screen.getByTestId('shield');
  ...
  it('should render UniSASE branding text', () => {
    ...
    expect(screen.getByText('统一网络与安全平台')).toBeInTheDocument();
  });
  ...
  it('should render enterprise SASE badge', () => {
    render(<Hero />);

    expect(screen.getByText('企业级 SASE 解决方案')).toBeInTheDocument();
  });
